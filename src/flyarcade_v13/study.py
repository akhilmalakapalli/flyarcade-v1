"""v1.3 study machinery: seed namespace, resumable PPO trainer, evaluation and probes."""
...
from flyarcade_v13.controller import FlyFeatures, SensoryFeatures, rng_for_episode
...
from flyarcade_v13.policy import ActorCritic, Adam
from flyarcade_v13.ppo import gae, sample_actions, update

# ------------------------------------------------------------------ seeds
SEED_BASE = 200_000_000
TASK_BLOCK = 100_000_000
PURPOSE_BLOCK = 5_000_000
SEED_SLOT = 250_000
MAX_SEEDS = PURPOSE_BLOCK // SEED_SLOT
...
PURPOSES = DEVELOPMENT_PURPOSES + CONFIRMATORY_PURPOSES
BEHAVIOUR_OFFSET = 2_000_000_000  # random-baseline action RNG, as in v1.2
...
def seed_for(task, purpose, seed=0, index=0):
    if (
        ...
        raise ValueError(f"invalid bounded v1.3 seed request: {task} {purpose} {seed} {index}")
    return (
        SEED_BASE
...
def code_files():
    return sorted(Path("src/flyarcade_v13").rglob("*.py")) + [Path("scripts/v13_trial.py")]
...
def standardizer_path(task, topology, ticks, readout):
    ...
    return Path(f"artifacts/v13/standardizers/{task}-{topology}-t{ticks}-{scope}.json")
...
def make_source(config, batch, graph=None):
    ...
    return FlyFeatures(
        graph, width, batch, ticks=config["ticks"], readout=config["readout"], standardizer=std
    )
...
def fit_standardizer(
    ...
    *,
    ...
    """
    width = TARGETS[task].width
    source = FlyFeatures(graph, width, batch, ticks=ticks, readout=readout)
    seeds = []
    samples = []
...
DEFAULTS = {
    ...
    "eval_every": 10,
    "curve_episodes": 16,
    "train_purpose": "screen_train",
    "eval_purpose": "screen_eval",
    "model_purpose": "dev_model",
    "rollout_purpose": "dev_rollout",
    "seed": 0,
    "learn": True,
}
...
def evaluate_policy(
    ...
    for start in range(0, len(seeds), batch):
        ...
        for c, env in enumerate(envs):
            rows[start + c] = _episode_record(env, actions[c], totals[c])
    return rows
